...
const BrandForm = () => {
  ...
  useEffect(() => {
    if (isNew) return;
    api.get('/brands', { params: { all: true } }).then((res) => {
      const b = res.data.find((x) => x.id === Number(id));
      if (b) setForm({ nameEn: b.nameEn, nameAr: b.nameAr, image: b.image || '', isActive: b.isActive, sortOrder: b.sortOrder });
      setLoaded(true);
    });
  }, [id, isNew]);
  ...
  return (
    <div>
      <div className="toolbar">
        <h1>{isNew ? t('brands.add_brand') : t('brands.edit_brand')}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ maxWidth: 560 }}>
        ...
        <div className="toolbar-actions">
          <button type="submit" className="btn" disabled={submitting}>
            {submitting ? t('common.saving') : isNew ? t('common.add') : t('brands.update')}
          </button>
